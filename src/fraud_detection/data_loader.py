...
#train data only since i'm builing and training the model 
import pandas as pd
...
def load_raw(raw_dir: str | Path) -> pd.DataFrame:
    ...
    return df
def reduce_memory(df: pd.DataFrame) -> pd.DataFrame: #scan the merged df and downcast numeric dtypes to save memory
